...
QVariant DropboxImageCacheModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == DropboxImageCacheModel::Image) {
        if (d->m_data.at(row).value(role).toString().isEmpty()) {
            // haven't downloaded the image yet.  Download it.
            if (d->database.images().size() > row) {
                DropboxImage::ConstPtr imageData = d->database.images().at(row);
                DropboxImageCacheModelPrivate *nonconstD = const_cast<DropboxImageCacheModelPrivate*>(d);
                nonconstD->queue(row, DropboxImageDownloader::FullImage,
                                 imageData->imageId(),
                                 imageData->imageUrl());
            }
        }
        ...
        const QString &, const QString &path, const QVariantMap &imageData)
{
    ...
    case DropboxImageDownloader::ThumbnailImage:
        ...
        break;
    case DropboxImageDownloader::FullImage:
        d->m_data[row].insert(DropboxImageCacheModel::Image, path);
        break;
    }
...
void DropboxImageCacheModel::queryFinished()
{
    ...
    case Images: {
        ...
        for (int i = 0; i < imagesData.count(); i ++) {
            ...
            imageMap.insert(DropboxImageCacheModel::Image, imageData->imageFile());
